...
const NODES: Node[] = [
  { id: "write",   label: "Write",   sub: "HNSW insert",       x: 0.08, y: 0.50 },
  { id: "encrypt", label: "Encrypt", sub: "AES-256-GCM",       x: 0.30, y: 0.20 },
  { id: "store",   label: "Store",   sub: "Arweave · Irys",    x: 0.55, y: 0.10 },
  { id: "anchor",  label: "Anchor",  sub: "SHA-256 Merkle",    x: 0.80, y: 0.20 },
  { id: "query",   label: "Query",   sub: "vector search",     x: 0.38, y: 0.80 },
  { id: "verify",  label: "Verify",  sub: "reconstruct state", x: 0.72, y: 0.80 },
  { id: "done",    label: "✓",       sub: "",                  x: 0.94, y: 0.50 },
];
...
export default function PipelineDemo() {
  ...
  function spawnWrite() {
    pulseRef.current.push({
      path: [0, 1, 2, 3, 6], seg: 0, t: 0,
      speed: 0.010 + Math.random() * 0.006,
      id: rh(4), done: false, type: "write",
    });
    writesRef.current++;
  ...
  function spawnQuery() {
    pulseRef.current.push({
      path: [0, 4, 6], seg: 0, t: 0,
      speed: 0.018 + Math.random() * 0.008,
      id: rh(4), done: false, type: "query",
    });
  }

  function spawnVerify() {
    pulseRef.current.push({
      path: [3, 5, 6], seg: 0, t: 0,
      speed: 0.008 + Math.random() * 0.004,
      id: rh(4), done: false, type: "verify",
    });
    proofsRef.current++;
  ...
  useEffect(() => {
    ...
    function drawNodes() {
      NODES.forEach((n) => {
        const x = n.x * W();
        const y = n.y * H;
        const col = n.id === "done" ? ACCENT_GREEN
          : n.id === "query" ? ACCENT_BLUE
          : n.id === "verify" ? ACCENT_GREEN
          : ACCENT;
    ...
    function drawPulses() {
      pulseRef.current.forEach((p) => {
        ...
        for (let i = 3; i >= 1; i--) {
          ctx.beginPath(); ctx.arc(x, y, i * 2.5, 0, Math.PI * 2);
      ...
      pulseRef.current.forEach((p) => {
        ...
        if (p.t >= 1) {
          ...
          if (p.type === "query" && arrived === 4) {
            ...
            const avg = (latenciesRef.current.reduce((a, b) => a + b, 0) / latenciesRef.current.length).toFixed(2) + "ms";
            setAvgLatency(avg);
            queriesRef.current++;
          }
          p.seg++;
          if (p.seg >= p.path.length - 1) { p.done = true; return; }
          p.speed = p.type === "query"
    ...
    function loop() {
      ...
      if (frame % 90 === 0 && verifyCount < 2 && writesRef.current > 3) spawnVerify();
  ...
  return (
    <section style={{ padding: "104px 0", borderBottom: "1px solid var(--rule, #1e1a14)" }}>
      <div style={{ maxWidth: "var(--maxw, 1200px)", margin: "0 auto", padding: "0 var(--gutter, 40px)" }}>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <h2 style={{
            fontFamily: "var(--font-serif, Georgia)", fontWeight: 400,
            fontSize: "clamp(28px, 3.5vw, 48px)", lineHeight: 1.1,
            letterSpacing: "-0.01em", color: "var(--paper, #f0ebe0)", margin: 0,
          }}>
            Every vector write.{" "}
            <em style={{ fontStyle: "italic", color: "var(--accent, #c2692a)" }}>Every proof.</em>
            <br />Instantly verifiable.
          </h2>
          <p style={{
            fontFamily: "var(--font-mono, ui-monospace)", fontSize: 13,
            color: "var(--text-dim, #666)", margin: "16px 0 0",
        ...
        <div style={{ background: BG, border: `1px solid ${BORDER}`, borderRadius: 12, overflow: "hidden" }}>
          <div style={{
            ...
            <span style={{ fontFamily: "ui-monospace,monospace", fontSize: 11, color: "#7a5a3e", letterSpacing: "0.14em", textTransform: "uppercase" }}>
              recall · agent memory pipeline
            </span>
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 10, color: ACCENT }}>
              <div style={{ width: 6, height: 6, borderRadius: "50%", background: ACCENT, animation: "recall-blink 1.4s infinite" }} />
              live
          ...
          <div style={{ display: "flex", borderTop: `1px solid ${BORDER}` }}>
            <div style={statStyle}>
              <div style={{ fontSize: 22, fontWeight: 700, color: "#fdf6ee", letterSpacing: "-0.02em", lineHeight: 1 }}>{writes.toLocaleString()}</div>
              <div style={{ fontSize: 9, color: "#5a3a20", letterSpacing: "0.1em", textTransform: "uppercase", marginTop: 4 }}>vectors written</div>
            </div>
            <div style={statStyle}>
              <div style={{ fontSize: 22, fontWeight: 700, color: ACCENT_BLUE, letterSpacing: "-0.02em", lineHeight: 1 }}>{avgLatency}</div>
              <div style={{ fontSize: 9, color: "#5a3a20", letterSpacing: "0.1em", textTransform: "uppercase", marginTop: 4 }}>avg query</div>
            </div>
            <div style={statStyle}>
              <div style={{ fontSize: 22, fontWeight: 700, color: ACCENT_GREEN, letterSpacing: "-0.02em", lineHeight: 1 }}>{proofs.toLocaleString()}</div>
              <div style={{ fontSize: 9, color: "#5a3a20", letterSpacing: "0.1em", textTransform: "uppercase", marginTop: 4 }}>proofs anchored</div>
            </div>
            <div style={{ ...statStyle, borderRight: "none" }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: ACCENT_GREEN, letterSpacing: "-0.02em", lineHeight: 1 }}>∞</div>
              <div style={{ fontSize: 9, color: "#5a3a20", letterSpacing: "0.1em", textTransform: "uppercase", marginTop: 4 }}>permanent</div>
            </div>
          </div>
        </div>
      </div>
